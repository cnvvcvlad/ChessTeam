...
<?php if ($this->isConnected()) : ?>
    ...
    <?php if (isset($params['pagination'])) : ?>
        <nav class="paginator">
            <li class="list_style page-item"><a href="<?= dirname(SCRIPTS) ?>/profile/posts?page=<?= $params['pagination']['currentPage'] -
                                                                                        1 ?>" class="<?= $params['pagination']['currentPage'] == 1
                                ? 'disabled'
                                : '' ?>">Précédente</a></li>
            <?php for ($page = 1; $page <= $params['pagination']['nbPages']; $page++) : ?>
                <li class="list_style page-item">
                    <a href="<?= dirname(SCRIPTS) ?>/profile/posts?page=<?= $page ?>" class="page-link-<?= $params['pagination']['currentPage'] ==
                                                                                                $page
                                                                                                ? 'active'
                                                                                                : '' ?>"><?= $page ?>
                    </a>
